...
class CamundaBpmVersion {
    ...
    protected static Pattern versionPattern = ~/^(\d+)\.(\d+)\.(\d+)-?(alpha|beta|Final|SNAPSHOT)?(\d*)(-ee)?$/
        ...
        if (actual > needed) {
            ...
            if (actual > needed) {
                ...
                if (actual > needed) {
                    ...
                    actual = (actualMatcher[0][4] ?: "Final") as String
                    needed = (neededMatcher[0][4] ?: "Final") as String
